...
import com.aridhu.gwt.pricing.client.FavoritesManager;
...
/**
 * Displays ProductItem objects that the user has selected as a favorite. This
...
public class FavoritesWidget extends Composite {
  ...
  /**
   * A driver that accepts a List of ProductItemProxy objects, controlled by a
   * ListEditor of ProductItemProxy instances, displayed using NameLabels.
   */
  interface Driver extends RequestFactoryEditorDriver<List<ProductItemProxy>, //
      ListEditor<ProductItemProxy, NameLabel>> {
  }
   ...
   */
  private final List<ProductItemProxy> displayedList;
  ...
  public FavoritesWidget(EventBus eventBus, RequestFactory factory,
    ...
    ListEditor<ProductItemProxy, NameLabel> editor = ListEditor.of(new NameLabelSource());

    // Configure the driver
    ListEditor<ProductItemProxy, NameLabel> listEditor = editor;
    driver.initialize(eventBus, factory, listEditor);

    // Notice the backing list is essentially anonymous.
    driver.display(new ArrayList<ProductItemProxy>());
  ...
  protected void onLoad() {
    ...
    for (EntityProxyId<ProductItemProxy> id : manager.getFavoriteIds()) {
  ...
  private void onMarkFavorite(MarkFavoriteEvent event) {
    EntityProxyId<ProductItemProxy> id = event.getId();
    if (id == null) {
      return;
    }

    // EntityProxies should be compared by stable id
    ProductItemProxy found = null;
    for (ProductItemProxy displayed : displayedList) {
    ...
    if (event.isFavorite() && found == null) {
      factory.find(id).to(new Receiver<ProductItemProxy>() {
        @Override
  ...
  private void sortDisplayedList() {
    // Sorting the list of ProductItemProxies will also change the UI display
    Collections.sort(displayedList, new Comparator<ProductItemProxy>() {
      public int compare(ProductItemProxy o1, ProductItemProxy o2) {
